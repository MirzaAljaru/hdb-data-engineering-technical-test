...
The integrated view covers all four requirements on one 16:9 presentation canvas. Numbered arrows identify the execution and transfer stages; solid lines show transfers/query connections and dashed lines show orchestration or metadata. The top strip summarises the processing sequence. The network map retains VPC and regional service boundaries, so raw replay and client-initiated query requests have arrows matching their actual direction.

The diagram shows four requirement callouts and a compact assumptions/retention footer. Detailed settings remain in this document. Use the integrated PNG for submission and the matching draw.io file for editing.
...
3. The data VPC spans two Availability Zones, with ASG-backed proxy capacity and one NAT gateway per AZ, plus local routes. Tableau is assumed to use a three-node cluster across three AZs, an internal ALB, and service endpoint ENIs in each AZ. Validate that topology against HDB's existing environment, budget, and Tableau licence. Batch tasks can be rescheduled after an AZ failure.
...
Ingestion tasks have no public IP. Their security group permits the egress proxy on TCP 3128, approved interface endpoints on TCP 443, and S3/DynamoDB service prefix lists on TCP 443. The proxy accepts requests only from ingestion workers and permits HTTPS CONNECT only to approved source hosts on port 443. The client validates source TLS certificates. The proxy is in a private egress subnet with a default route through the same-AZ NAT gateway in a public subnet; that subnet routes outbound internet traffic through the VPC internet gateway. The Fargate client initiates the connection. Source responses return through that established connection; data.gov.sg does not initiate an inbound connection to HDB.

The proxy block represents two private proxy services, each with an ASG maintaining at least one EC2 instance in its AZ. Use an internal per-AZ frontend and application health checks so clients can reach replacement instances without hardcoded IP addresses. Configure ASG health handling and worker connection retries; verify source allowlists remain effective after replacement. The diagram aggregates this frontend with the proxy service. There is one NAT gateway per AZ and one VPC-attached internet gateway. [ASG health and replacement](https://docs.aws.amazon.com/autoscaling/ec2/userguide/health-checks-overview.html)

The proxy subnet deliberately has no S3 gateway endpoint route. This keeps publicly supplied S3 download URLs on the controlled source egress path rather than subjecting them to an endpoint policy restricted to HDB buckets. Worker subnet route tables have local S3 and DynamoDB gateway routes. ETL workers cannot reach the proxy and have no internet default route. Interface endpoints provide ECR API/DKR, CloudWatch Logs, Secrets Manager, and Glue access. The S3 endpoint policy also permits the ECR image-layer bucket required by Fargate. The diagram's `ECS*` denotes an optional ECS API interface endpoint for in-VPC clients that call ECS APIs. Fargate itself does not require ECS interface endpoints. [Fargate endpoint requirements](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/vpc-endpoints.html)
...
The presentation view aggregates Tableau into a private HA cluster behind an internal ALB. The sizing assumption is three EC2 nodes in separate AZs, with redundant Coordination Service, Client File Service, repository, File Store, and query/gateway processes. Validate the precise topology against HDB's existing installation, recovery needs, latency, and Tableau licence before implementation. The ALB routes private HTTPS traffic to healthy gateways. Configure inter-node access using the deployed release's documented port matrix. This is a proposed cluster topology, not independent single-node installations. [Tableau HA requirements](https://help.tableau.com/current/server-linux/en-us/distrib_ha_install_3node.htm)
